...
%% ----------------- Main Func -------------------- %%
function testCorrectionRatios_v2
close all;
clc;
...
for k=stopAtRangeL:1:stopAtRangeU
    ...
    fpr_per = 0; mpr_per = 0; cr_per = 0;
    fpr_unper = 0; mpr_unper = 0; cr_unper = 0;
    
    avgRes_per = 0; avgRes_unper = 0;
    ...
    while i<=numTestProb
        ...
        %% Solve for 'actual active-set'
        [m, n] = size(A);
        
        %  Solve the problem using linprog (simplex) and get the actual actv
        %  for per alg
        [actualActv_per,   exitflag_per  ] = solveLinprog(A,b,c, 'splx');
        
        %  Solve the problem using linprog (interior) and get the actual actv
        %  for unper alg
        [actualActv_unper, exitflag_unper] = solveLinprog(A,b,c, 'ipm' );
        ...
        [tpfpr_per, tpmpr_per, tpcr_per] = ...
            getCorrectionRatio(per.getActv, actualActv_per);
        [tpfpr_unper, tpmpr_unper, tpcr_unper] = ...
            getCorrectionRatio(unper.getActv, actualActv_unper);
        
        fpr_per  = fpr_per+tpfpr_per;    fpr_unper = fpr_unper+tpfpr_unper;
        mpr_per  = mpr_per+tpmpr_per;    mpr_unper = mpr_unper+tpmpr_unper;
        cr_per   = cr_per+tpcr_per;      cr_unper  = cr_unper+tpcr_unper;
        
        avgRes_per   = per.getIPMResidual + avgRes_per;
        avgRes_unper = unper.getIPMResidual   + avgRes_unper;
    ...
    %% Get the averages
    falsePrediction(counter,:)  = [fpr_per fpr_unper]./numTestProb;
    missedPrediction(counter,:) = [mpr_per mpr_unper]./numTestProb;
    correctionR(counter,:)      = [cr_per cr_unper]./numTestProb;
    
    avgResidual(counter,:)      = [avgRes_per avgRes_unper]./numTestProb;
