classdef pipm < handle
    ...
    %                            maxIter, tol, mu_cap, iPer, cutoff, verbose,
    %                            actvPredStrtgy.
    %
    % September 16 2013
    % Yiming Yan
    ...
    methods
        ...
        function solve(p)
            ...
            while ~p.iter.checkTermination(p.counter, p.parameters, p.status);
                ...
                % iter_x(:,p.counter.iterN+1) = p.prob.x; iter_s(:,p.counter.iterN+1) = p.prob.s; iter_y(:,p.counter.iterN+1) = p.prob.y; save('iter_xys.mat', 'iter_x', 'iter_y', 'iter_s');
